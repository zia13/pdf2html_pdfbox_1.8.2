...
//    File fiile = new File("D:/zia.txt"); 
    List<TextPosition> TextinArea = null;
//    BufferedWriter outputt = new BufferedWriter(new FileWriter(fiile));
    ...
    public PDFTextStripper(int l) throws IOException
    {        
        super( ResourceLoader.loadProperties(
                "org/apache/pdfbox/resources/PDFTextStripper.properties", true ) );
        forColumn = l;
...
//        System.out.println(text+"---- (X,Y): ("+text.getX()+","+text.getY()+")");
    }
...
//        System.out.println(text);//+"---- (X,Y): ("+text.getX()+","+text.getY()+")");
    ...
    private void writeLine(List<String> line, boolean isRtlDominant)throws IOException
    {
        ...
        for(int i=0; i<numberOfStrings; i++)
        {
            ...
            writeString(line.get(i));
            String SingleLine = line.get(i);
            String[] word = SingleLine.split(" ");
        }
    }
    
    public List<TextPosition> getAllSelectedText()
    {
        return TextinArea;
    }
     ...
     */
    String createColumnText;// = new String[50];
    
    public String getColumntext()
    {
        return createColumnText;
    }
    
    public TextPosition getLeftLetter()
    {
        return leftLetter;
    }
    
    public TextPosition getRightLetter()
    {
        return rightLetter;
    }
    
    public void setLeftLetter(TextPosition ll)
    {
        leftLetter = ll;
    }
    
    public void setRightLetter(TextPosition rl)
    {
        rightLetter = rl;
    }
    
    TextPosition leftLetter = new TextPosition();
    TextPosition rightLetter = new TextPosition();
    
    public List<String> normalize(List<TextPosition> line, boolean isRtlDominant, boolean hasRtl)
    {
        LinkedList<String> normalized = new LinkedList<String>();
        StringBuilder lineBuilder = new StringBuilder();
	int startat = 0;
        TextPosition ll = new TextPosition();
        TextPosition rl = new TextPosition();
        if (isRtlDominant)
        {
            ...
            for(int i = numberOfPositions-1;i>=0;i--)
            {
                ...
                {
                    if(startat == 0 && !"(".equals(text.getCharacter()))
                    {
                        ll = text;
                        startat++;
                    }                
                ...
                {
                    if(startat == 0 && !"(".equals(text.getCharacter()) && !" ".equals(text.getCharacter()))
                    {
                        ll = text;
                        startat++;
                    }                
        ...
        setLeftLetter(ll);
        setRightLetter(rl);
//        System.out.println("Left letter:"+ll.getCharacter()+", Right Letter: "+rl.getCharacter());
        createColumnText = lineBuilder.toString();
        return normalized;
    }
